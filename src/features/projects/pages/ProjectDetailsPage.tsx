...
export function ProjectDetailsPage() {
  ...
  const isUser = Boolean(
    project.user_id && session?.user.id && session?.user.id === project.user_id
  );

  return (
    <Stack direction={{ base: "column", lg: "row" }} gap={{ base: 4, md: 12 }}>
      <Stack flex={1}>
        {isMobile && <Heading>{project.title}</Heading>}
        <Box position={"relative"}>
          {session && (
            <Box position="absolute" top="2" right="2" zIndex="1">
              <LikeButton project={project} isLikedByUser={isLikedByUser} />
            </Box>
          )}
          <Image
            fit={"cover"}
      ...
      <Stack flex={1} gap={4}>
        ...
        <Separator />
        <Stack gap={"inherit"} direction={{ base: "column", md: "row" }}>
          {isUser && (
            <>
              <Button asChild variant={"outline"}>
                <RouterLink to={`/project/edit/${project.id}`}>Edit</RouterLink>
              </Button>
